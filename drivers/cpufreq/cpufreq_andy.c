/*
 * drivers/cpufreq/cpufreq_andy.c
 *
 * Copyright (C) 2011-2014 The XPerience PRoject
 * All of this work is from TeamMEX@XDA this is just a copy for andy kernel MOTO E
...
static DEFINE_PER_CPU(struct andy_info_s, andy_info);
...
static int cpufreq_governor_andy(struct cpufreq_policy *policy,
		unsigned int event);

#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_ANDY
static
#endif
struct cpufreq_governor cpufreq_gov_andy = {
	.name = "andy",
	.governor = cpufreq_governor_andy,
	.max_transition_latency = 9000000,
	.owner = THIS_MODULE,
};

inline static void andy_update_min_max(struct andy_info_s *this_andy, struct cpufreq_policy *policy, int suspend) {
	if (suspend) {
		this_andy->ideal_speed = // sleep_ideal_freq; but make sure it obeys the policy min/max
			policy->max > sleep_ideal_freq ?
			(sleep_ideal_freq > policy->min ? sleep_ideal_freq : policy->min) : policy->max;
	} else {
		this_andy->ideal_speed = // awake_ideal_freq; but make sure it obeys the policy min/max
			policy->min < awake_ideal_freq ?
			(awake_ideal_freq < policy->max ? awake_ideal_freq : policy->max) : policy->min;
	}
}

inline static void andy_update_min_max_allcpus(void) {
	unsigned int i;
	for_each_online_cpu(i) {
		struct andy_info_s *this_andy = &per_cpu(andy_info, i);
		if (this_andy->enable)
			andy_update_min_max(this_andy,this_andy->cur_policy,suspended);
	}
}
...
inline static void reset_timer(unsigned long cpu, struct andy_info_s *this_andy) {
	this_andy->time_in_idle = get_cpu_idle_time_us(cpu, &this_andy->idle_exit_time);
	mod_timer(&this_andy->timer, jiffies + sample_rate_jiffies);
}
...
inline static int target_freq(struct cpufreq_policy *policy, struct andy_info_s *this_andy,
			      int new_freq, int old_freq, int prefered_relation) {
	int index, target;
	struct cpufreq_frequency_table *table = this_andy->freq_table;
	    ...
	    !cpufreq_frequency_table_target(policy,table,new_freq,prefered_relation,&index))
	{
		...
		if (target == old_freq) {
			...
			printk(KERN_WARNING "andy: frequency change failed: %d to %d => %d\n",
	...
	dprintk(ANDY_DEBUG_JUMPS,"andyQ: jumping from %d to %d => %d (%d)\n",
		old_freq,new_freq,target,policy->cur);

	return target;
}

static void cpufreq_andy_timer(unsigned long cpu)
{
	u64 delta_idle;
	u64 delta_time;
	int cpu_load;
	...
	u64 now_idle;
	int queued_work = 0;
	struct andy_info_s *this_andy = &per_cpu(andy_info, cpu);
	struct cpufreq_policy *policy = this_andy->cur_policy;

	now_idle = get_cpu_idle_time_us(cpu, &update_time);
	old_freq = policy->cur;

	if (this_andy->idle_exit_time == 0 || update_time == this_andy->idle_exit_time)
		return;

	delta_idle = cputime64_sub(now_idle, this_andy->time_in_idle);
	delta_time = cputime64_sub(update_time, this_andy->idle_exit_time);

	// If timer ran less than 2ms after short-term sample started, retry.
	if (delta_time < 2000) {
		if (!timer_pending(&this_andy->timer))
			reset_timer(cpu,this_andy);
		return;
	}
	...
	dprintk(ANDY_DEBUG_LOAD,"andyT @ %d: load %d (delta_time %llu)\n",
		old_freq,cpu_load,delta_time);

	this_andy->cur_cpu_load = cpu_load;
	this_andy->old_freq = old_freq;
	...
	if (cpu_load > max_cpu_load || delta_idle == 0)
	{
		if (old_freq < policy->max &&
			 (old_freq < this_andy->ideal_speed || delta_idle == 0 ||
			  cputime64_sub(update_time, this_andy->freq_change_time) >= up_rate_us))
		{
			dprintk(ANDY_DEBUG_ALG,"andyT @ %d ramp up: load %d (delta_idle %llu)\n",
				old_freq,cpu_load,delta_idle);
			this_andy->ramp_dir = 1;
			work_cpumask_set(cpu);
			queue_work(up_wq, &freq_scale_work);
			queued_work = 1;
		}
		else this_andy->ramp_dir = 0;
	}
	...
	// frequency we require that we have been at this frequency for at least down_rate_us:
		 ...
		 (old_freq > this_andy->ideal_speed ||
		  cputime64_sub(update_time, this_andy->freq_change_time) >= down_rate_us))
	{
		dprintk(ANDY_DEBUG_ALG,"andyT @ %d ramp down: load %d (delta_idle %llu)\n",
			old_freq,cpu_load,delta_idle);
		this_andy->ramp_dir = -1;
		work_cpumask_set(cpu);
...
{
	struct andy_info_s *this_andy = &per_cpu(andy_info, smp_processor_id());
	struct cpufreq_policy *policy = this_andy->cur_policy;
	...
	if (policy->cur == policy->min && timer_pending(&this_andy->timer))
		del_timer(&this_andy->timer);

	pm_idle_old();

	if (!timer_pending(&this_andy->timer))
		reset_timer(smp_processor_id(), this_andy);
}
...
static int cpufreq_idle_notifier(struct notifier_block *nb,
	...
	if (val == IDLE_START) {
		if (policy->cur == policy->max && !timer_pending(&this_andy->timer)) {
			reset_timer(smp_processor_id(), this_andy);
...
static void cpufreq_andy_freq_change_time_work(struct work_struct *work)
{
	unsigned int cpu;
	int new_freq;
	int old_freq;
	int ramp_dir;
	struct andy_info_s *this_andy;
	struct cpufreq_policy *policy;
	unsigned int relation = CPUFREQ_RELATION_L;
	for_each_possible_cpu(cpu) {
		this_andy = &per_cpu(andy_info, cpu);
		if (!work_cpumask_test_and_clear(cpu))
			continue;

		ramp_dir = this_andy->ramp_dir;
		this_andy->ramp_dir = 0;

		old_freq = this_andy->old_freq;
		policy = this_andy->cur_policy;

		if (old_freq != policy->cur) {
			// frequency was changed by someone else?
			printk(KERN_WARNING "andy: frequency changed by 3rd party: %d to %d\n",
			       old_freq,policy->cur);
			new_freq = old_freq;
		}
		else if (ramp_dir > 0 && nr_running() > 1) {
			// ramp up logic:
			if (old_freq < this_andy->ideal_speed)
				new_freq = this_andy->ideal_speed;
			...
			dprintk(ANDY_DEBUG_ALG,"andyQ @ %d ramp up: ramp_dir=%d ideal=%d\n",
				old_freq,ramp_dir,this_andy->ideal_speed);
		}
		else if (ramp_dir < 0) {
			// ramp down logic:
			if (old_freq > this_andy->ideal_speed) {
				...
				new_freq = old_freq * this_andy->cur_cpu_load / max_cpu_load;
				if (new_freq > old_freq) // min_cpu_load > max_cpu_load ?!
					new_freq = old_freq -1;
			}
			dprintk(ANDY_DEBUG_ALG,"andyQ @ %d ramp down: ramp_dir=%d ideal=%d\n",
				old_freq,ramp_dir,this_andy->ideal_speed);
		}
			...
			new_freq = old_freq;
			dprintk(ANDY_DEBUG_ALG,"andyQ @ %d nothing: ramp_dir=%d nr_running=%lu\n",
				old_freq,ramp_dir,nr_running());
		}

		// do actual ramp up (returns 0, if frequency change failed):
		new_freq = target_freq(policy,this_andy,new_freq,old_freq,relation);
		if (new_freq)
			this_andy->freq_change_time_in_idle =
				get_cpu_idle_time_us(cpu,&this_andy->freq_change_time);
		...
		else if (timer_pending(&this_andy->timer))
			del_timer(&this_andy->timer);

		cpufreq_notify_utilization(policy,
			(this_andy->cur_cpu_load * policy->cur) / policy->max);
	}
}
...
static struct attribute * andy_attributes[] = {
...
static struct attribute_group andy_attr_group = {
	.attrs = andy_attributes,
	.name = "andy",
};

static int cpufreq_governor_andy(struct cpufreq_policy *new_policy,
		unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		this_andy->cur_policy = new_policy;

		this_andy->enable = 1;

		andy_update_min_max(this_andy,new_policy,suspended);

		this_andy->freq_table = cpufreq_frequency_get_table(cpu);
		if (!this_andy->freq_table)
			printk(KERN_WARNING "andy: no frequency table for cpu %d?!\n",cpu);
		...
		if (this_andy->cur_policy->cur < new_policy->max && !timer_pending(&this_andy->timer))
			reset_timer(cpu,this_andy);
	...
	case CPUFREQ_GOV_LIMITS:
		...
		if (this_andy->cur_policy->cur > new_policy->max) {
			dprintk(ANDY_DEBUG_JUMPS,"andyI: jumping to new max freq: %d\n",new_policy->max);
			__cpufreq_driver_target(this_andy->cur_policy,
						new_policy->max, CPUFREQ_RELATION_H);
		}
		else if (this_andy->cur_policy->cur < new_policy->min) {
			dprintk(ANDY_DEBUG_JUMPS,"andyI: jumping to new min freq: %d\n",new_policy->min);
			__cpufreq_driver_target(this_andy->cur_policy,
...
static void andy_suspend(int cpu, int suspend)
{
	struct andy_info_s *this_andy = &per_cpu(andy_info, smp_processor_id());
	struct cpufreq_policy *policy = this_andy->cur_policy;
	...
	andy_update_min_max(this_andy,policy,suspend);
	if (!suspend) { // resume at max speed:
		new_freq = validate_freq(policy,sleep_wakeup_freq);

		dprintk(ANDY_DEBUG_JUMPS,"andyS: awaking at %d\n",new_freq);
		...
		dprintk(ANDY_DEBUG_JUMPS,"andyS: suspending at %d\n",policy->cur);
	}

	reset_timer(smp_processor_id(),this_andy);
}

static void andy_early_suspend(struct power_suspend *handler) {
	int i;
...
static void andy_late_resume(struct power_suspend *handler) {
	int i;
...
static struct power_suspend andy_power_suspend = {
	.suspend = andy_early_suspend,
	.resume = andy_late_resume,
};

static int __init cpufreq_andy_init(void)
{
	unsigned int i;
	struct andy_info_s *this_andy;
	debug_mask = 0;
	...
	for_each_possible_cpu(i) {
		this_andy = &per_cpu(andy_info, i);
		this_andy->enable = 0;
		this_andy->cur_policy = 0;
		this_andy->ramp_dir = 0;
		this_andy->time_in_idle = 0;
		this_andy->idle_exit_time = 0;
		this_andy->freq_change_time = 0;
		this_andy->freq_change_time_in_idle = 0;
		this_andy->cur_cpu_load = 0;
		// intialize timer:
		init_timer_deferrable(&this_andy->timer);
		this_andy->timer.function = cpufreq_andy_timer;
		this_andy->timer.data = i;
		work_cpumask_test_and_clear(i);
	}

	// Scale up is high priority
	up_wq = create_workqueue("kandy_up");
	down_wq = create_workqueue("kandy_down");
	if (!up_wq || !down_wq)
		return -ENOMEM;

	INIT_WORK(&freq_scale_work, cpufreq_andy_freq_change_time_work);

	register_power_suspend(&andy_power_suspend);

	return cpufreq_register_governor(&cpufreq_gov_andy);
}

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_ANDY
fs_initcall(cpufreq_andy_init);
#else
module_init(cpufreq_andy_init);
#endif

static void __exit cpufreq_andy_exit(void)
{
	cpufreq_unregister_governor(&cpufreq_gov_andy);
	destroy_workqueue(up_wq);
	destroy_workqueue(down_wq);
}

module_exit(cpufreq_andy_exit);

MODULE_AUTHOR ("Carlos Jesús B.C (Klozz-TeamMEX)<xxx.reptar.rawrr.xxx@gmail.com>");
MODULE_DESCRIPTION ("'cpufreq_Andy' - A smart cpufreq governor");
MODULE_LICENSE ("GPL");
